...
const DataAllPenyakit = ({ dataPenyakit }) => {
  const router = useRouter();

  const handleEditClick = (tdata) => {
    router.push(`/admin/data-penyakit?id=${tdata.id}`);
  };
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (confirmDelete) {
      try {
        ...
        await deleteDoc(doc(db, "dataPenyakit", id));
...
export default DataAllPenyakit;
