...
const DataAllGejala = ({ dataGejala, dataPenyakit }) => {
  const router = useRouter();

  const handleEditClick = (tdata) => {
    router.push(`/admin/data-gejala?id=${tdata.id}`);
  };
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (confirmDelete) {
      try {
        ...
        await deleteDoc(doc(db, "dataGejala", id));
        ...
        <Table className="text-nowrap mt-3 align-middle" bordered>
          <thead>
            <tr className="text-center">
              ...
              <th colSpan={6}>Kode Penyakit </th>
              <th rowSpan={2}>Bobot Pakar </th>
              <th rowSpan={2}>Aksi</th>
            </tr>
            <tr className="text-center">
              {dataPenyakit.map((p, inde) => (
                <th key={inde}>{p.kodePenyakit}</th>
              ))}
            </tr>
          </thead>
          {dataGejala.map((tdata, index) => (
            <tbody key={index}>
              <tr className="border-top text-center">
                <td>{tdata.kodeGejala}</td>
                <td className="text-start">
                  {tdata.namaGejala}
                </td>
                {dataPenyakit.map((p, inde) => (
                  <td key={inde}>
                    {tdata.penyakitTerkait.includes(p.kodePenyakit) ? '✔' : ''}
                  </td>
                ))}
                <td>{tdata.bobotPakar}</td>
                <td>
